...
const SHEET_URL = 'https://docs.google.com/spreadsheets/d/1vvE_PB_zGDBslHw29xvLVQWGQBUmuDZG/export?format=csv';
...
export default function App() {
  ...
  const designations = useMemo(() => {
    const unique = new Set(officers.map(o => o.designation));
  ...
  const filteredOfficers = useMemo(() => {
    return officers.filter(officer => {
      ...
      const matchesFilter = designationFilter === 'all' || officer.designation === designationFilter;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <AnimatePresence mode="popLayout">
              {filteredOfficers.map((officer, index) => (
                ...
                >
                  <Card className="glass-card h-full border-none rounded-[2rem] overflow-hidden group hover:-translate-y-2 transition-all duration-500">
                    <div className="h-2 w-full bg-gradient-to-r from-purple-500 to-pink-500 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                    ...
                    <CardHeader className="pb-4 pt-8 px-8">
                      <div className="flex flex-col items-center text-center gap-4">
                        ...
                        <div className="space-y-1">
                          <CardTitle className="text-2xl font-display font-bold text-slate-900 group-hover:text-purple-600 transition-colors">
                            {officer.name}
                          </CardTitle>
                          <div className="inline-flex px-3 py-1 rounded-full bg-purple-50 text-purple-700 text-xs font-bold uppercase tracking-wider">
                            {officer.designation}
                          </div>
                        </div>
                      </div>
                    ...
                    <CardContent className="pt-4 pb-8 px-8 space-y-6">
                      <div className="space-y-4">
                        <a 
                          ...
                          className="flex items-center gap-4 p-3 rounded-2xl bg-slate-50/50 hover:bg-purple-50 hover:text-purple-700 transition-all group/link"
                        >
                          <div className="p-2 bg-white rounded-xl shadow-sm group-hover/link:shadow-purple-100 transition-all">
                            <Phone className="h-4 w-4 text-slate-400 group-hover/link:text-purple-500" />
                          </div>
                        ...
                        >
                          <div className="p-2 bg-white rounded-xl shadow-sm group-hover/link:shadow-purple-100 transition-all">
                            <Mail className="h-4 w-4 text-slate-400 group-hover/link:text-purple-500" />
                          </div>
                      ...
                      <a 
                        ...
                        className={cn(
                          ...
                          "w-full h-12 rounded-2xl bg-slate-900 hover:bg-gradient-to-r hover:from-purple-500 hover:to-pink-500 text-white shadow-lg shadow-slate-200 hover:shadow-purple-200 transition-all duration-300 active:scale-[0.98] font-bold flex items-center justify-center"
                        )}
                      >
